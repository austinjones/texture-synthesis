...
pub struct GeneratedImage<P: SynthPixel> {
    inner: multires_stochastic_texture_synthesis::Generator<P>,
}

impl<P: SynthPixel> GeneratedImage<P> {
    /// Saves the generated image to the specified path
    pub fn save<A: AsRef<Path>>(&self, path: A) -> Result<(), Error> {
    ...
    pub fn write<W: std::io::Write>(
        ...
        let dynamic_image = P::into_dynamic_image(self.into_image());
        Ok(dynamic_image.write_to(writer, fmt)?)
    }
    ...
    pub fn save_debug<A: AsRef<Path>>(&self, dir: A) -> Result<(), Error> {
        let dir = dir.as_ref();
    ...
    pub fn into_image(self) -> SynthPixelBuffer<P> {
        self.inner.color_map
    }
}

impl<P: SynthPixel> AsRef<SynthPixelBuffer<P>> for GeneratedImage<P> {
    fn as_ref(&self) -> &SynthPixelBuffer<P> {
        &self.inner.color_map
    }
}
...
impl<'a> Example<'a> {
    ...
    fn resolve<P: SynthPixel>(
        self,
        backtracks: u32,
        resize: Option<(u32, u32)>,
        target_guide: &Option<ImagePyramid<P>>,
    ) -> Result<ResolvedExample<P>, Error> {
        let image = ImagePyramid::new(load_image(self.img, resize)?, Some(backtracks));

        let guide: Option<ImagePyramid<P>> = match target_guide {
            Some(tg) => {
...
impl<'a> SessionBuilder<'a> {
    ...
    pub fn build<P: SynthPixel>(self) -> Result<Session<P>, Error> {
        self.check_parameters_validity()?;
        self.check_images_validity()?;

        let target_guide: Option<ImagePyramid<P>> = match self.target_guide {
            Some(tg) => {
                let tg_img = load_image::<P>(tg, Some(self.params.output_size))?;
        ...
        struct InpaintExample<P: SynthPixel> {
            inpaint_mask: SynthPixelBuffer<P>,
        ...
        let inpaint = match self.inpaint_mask {
            Some((src, ind)) => {
                let inpaint_mask = load_image::<P>(src, Some(self.params.output_size))?;
                let color_map = load_image::<P>(
...
struct ResolvedExample<P: SynthPixel> {
    image: ImagePyramid<P>,
    guide: Option<ImagePyramid<P>>,
...
/// ```
pub struct Session<P: SynthPixel> {
    examples: Vec<ImagePyramid<P>>,
    guides: Option<GuidesPyramidStruct<P>>,
    sampling_methods: Vec<SamplingMethod>,
    generator: Generator<P>,
    params: Parameters,
}

impl<P: SynthPixel> Session<P> {
    ...
    pub fn run(mut self, progress: Option<Box<dyn GeneratorProgress<P>>>) -> GeneratedImage<P> {
        // random resolve
        ...
        if let Some(count) = self.params.random_resolve {
            ...
            let imgs: Vec<&SynthPixelBuffer<P>> = self
                .examples
                .iter()
                .map(|a| &a.pyramid[lvl - 1]) //take the blurriest image
                .collect();

            self.generator
                .resolve_random_batch(count as usize, imgs.as_slice(), self.params.seed);
        }
...
pub struct ProgressUpdate<'a, P: SynthPixel> {
    /// The currenty resolved image
    pub image: &'a SynthPixelBuffer<P>,
...
pub trait GeneratorProgress<P: SynthPixel> {
    fn update(&mut self, info: ProgressUpdate<'_, P>);
}

impl<G, P: SynthPixel> GeneratorProgress<P> for G
where
    G: FnMut(ProgressUpdate<'_, P>) + Send,
{
    fn update(&mut self, info: ProgressUpdate<'_, P>) {
        self(info)
    }
